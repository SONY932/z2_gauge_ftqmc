...
nbin=7500
nsweep=1
max_cpus_per_node=4  # 每个节点的最大核心数为56
...
run_group() {
    ...
    (
        ...
        # 仅 L=12（已在 L_list 中约束，这里保留循环以兼容行为）
        for ((i=${#L_list[@]}-1; i>=0; i--)); do
            L=${L_list[$i]}
            cpu=1
...
# is_global, Nglobal
# absolute
# sector_charge(扇区电荷±1)
# dtau_guard, zero_fix 守卫与扩展修正（可选行，缺省关闭）
EOF
